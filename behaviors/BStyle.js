...
export class BStyle extends Behavior {
  ...
  constructor() {
    ...
    this.defineMethod("config", (that, mainElement) => {
      ...
      let defineAttribute = (root, attributes, getter, setter, propName) => {
        if (attributes.length === 1) {
          if (attributes[0] in root) {
            let descriptor = Object.getOwnPropertyDescriptor(root, attributes[0]);
      ...
      forEach(allStylesList, (propName) => {
        ...
        let propNameClear = propName.replace(/^-*(\w[\w-]*\w)-*$/, "$1");
        let attrs = propNameClear.split("-");
        // console.log(propName, propNameClear, attrs);

        // let propDashName = attrs.join("-");
